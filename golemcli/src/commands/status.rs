...
impl Section {
    ...
    pub fn status(
        ...
        let x = online_nodes.from_err().join4(
            connection_status.from_err(),
            component_status.from_err(),
            task_stats.from_err(),
        );
...
impl FormattedGeneralStatus {
    ...
    fn print_component_status(&self, _: &mut io::Stdout) {
        let docker_status = self.components_status.docker.as_ref().map_or_else(
            || "",
            |component_report| map_statuses("docker", &component_report.0, &component_report.1),
        );

        println!("Component status = {:?}", self.components_status);
        println!("docker status = {:?} = {}", self.components_status.docker, docker_status);
    }
}
